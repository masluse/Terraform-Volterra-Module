    ...
    content {
      dynamic "match" {
        ...
        content {
          http_method = "ANY"
          path {
            path = replace(replace(replace(replace(replace(replace(match.value.path, " ", "%20"), "ä", "%C3%A4"), "ö", "%C3%B6"), "ü", "%C3%BC"), "é", "%C3%A9"), "à", "%C3%A0")
          }
          dynamic "query_params" {
            for_each = match.value.query_params
            content {
              key   = query_params.key
              exact = replace(replace(replace(replace(replace(replace(query_params.value, " ", "%20"), "ä", "%C3%A4"), "ö", "%C3%B6"), "ü", "%C3%BC"), "é", "%C3%A9"), "à", "%C3%A0")
            }
          }
          dynamic "headers" {
